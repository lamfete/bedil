...
class Reportexec extends CI_Controller {
    ...
    /*
     * Function untuk mendapatkan last stock
     * 
        ...
        if($this->session->userdata('isLogin') == FALSE) {
            ...
            if($type == 'datatables') {
                ...
                if(empty($_POST['search']['value'])) {
                    $data['all_acc_payable'] = $this->m_report->get_last_stock("all", $data['parameter']);  
                    $data['all_acc_payable']->draw = $_POST['draw'];
                }
                else {
                    $data['all_acc_payable'] = $this->m_report->get_last_stock("search", $data['parameter']);
                    $data['all_acc_payable']->draw = $_POST['draw'];
                }
            } elseif($type == 'autocomplete') {
                $data['all_acc_payable'] = $this->m_report->get_last_stock("autocomplete", $_GET['query']);
            }
     ...
     * 
     */
    public function get_omzet_stock() {
        // $data['all_user'] = new \stdClass();
        $type = $_GET['type'];
        if($this->session->userdata('isLogin') == FALSE) {
            redirect('login/form');
            ...
            if($type == 'datatables') {
                ...
                if(empty($_POST['search']['value'])) {
                    $data['all_acc_payable'] = $this->m_report->get_omzet_stock("all", $data['parameter']);  
                    $data['all_acc_payable']->draw = $_POST['draw'];
                }
                else {
                    $data['all_acc_payable'] = $this->m_report->get_omzet_stock("search", $data['parameter']);
                    $data['all_acc_payable']->draw = $_POST['draw'];
                }
            } elseif($type == 'autocomplete') {
                $data['all_acc_payable'] = $this->m_report->get_last_stock("autocomplete", $_GET['query']);
